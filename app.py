...
Built with Streamlit + Anthropic Claude.
...
with st.sidebar:
    st.header("Settings")
    api_key = st.text_input("Anthropic API Key", type="password", key="anthropic_api_key_home")
    if api_key:
        st.session_state["anthropic_api_key"] = api_key
    if st.session_state.get("anthropic_api_key"):
        st.success("API key set")
    else:
        st.warning("Enter your Anthropic API key to enable AI feedback. Get one at https://console.anthropic.com/")
...
st.markdown(
    ...
    *Built for the Dr. Perelman AIxEducation Project. Powered by Anthropic Claude.*
    """
)
